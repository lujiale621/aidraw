...
def api_only():
    ...
    def aimalltrainlora(train_data: dict):
        trainout_folder_name=train_data.get("trainout_folder_out")
        train_folder_name = train_data.get("train_folder_name")
        ...
        for j in range(len(train_images[0])):
            ...
            decode_base64_to_file(train_images[0][j], image_path)

        return {"message": "Images saved successfully."}
    def aimalltraindeambooth(train_data: dict, reg_data: dict):
        train_folder_name = train_data.get("train_folder_name")
        reg_folder_name = reg_data.get("reg_folder_name")
        trainout_folder_name =  train_data.get("train_out")
    ...
    def aimalltraindeamboothorlora(base_model,isdeambooth:bool,train_data: dict,reg_data: dict):
        ...
        if isdeambooth:
            ...
            aimalltrainlora(train_data)
            batch_size=4
            train_data_dir=train_data.get('train_folder_name')
            cmd = ["python", "/home/aimall/lujiale/stable-diffusion-webui/sd_scripts/finetune/tag_images_by_wd14_tagger.py", "--batch_size", str(batch_size), train_data_dir]
            subprocess.run(cmd)
        print("ok")
    ...
    api.launch(server_name="0.0.0.0" if cmd_opts.listen else "192.168.101.71", port=cmd_opts.port if cmd_opts.port else 7861)
